...
def train_and_test_model(model):
    ...
    if model == 'rf':
        ...
        run_tabtransformer(train_set, val_set, test_set, variables)
        return
    else:
...
def run_tabtransformer(train_set, val_set, test_set, variables):
    num_cols = variables
    ...
    y_proba_1 = pred_df['1_probability'].values
    ...
    model_path = os.path.join('storage', '{}_benchmark_fold{}'.format(args.model, args.fold))
    if not os.path.exists(model_path):
        os.makedirs(model_path)
    # save model
...
if __name__ == '__main__':
    ...
    train_and_test_model(args.model)