...
class GraphRankFormer(torch.nn.Module):
    def __init__(self, input_dim,
        ...
        if loss_fn == "softmax":
            ...
            self.eval_loss_fn = GraphBasedLoss(train_mode=False)
        elif loss_fn == 'pasoftmax':
            self.rank_loss_fn = PASoftmaxLoss()
        elif loss_fn == 'pabce':
            self.rank_loss_fn = PACrossEntropyLoss()
        elif loss_fn == 'bce':
            self.rank_loss_fn = CrossEntropyLoss()
        elif loss_fn == "lambdaloss":
    ...
    def compute_loss(self, score, target, length, pnl=None, train_mode=True, binary=True):
        """
        ...
        if pnl is not None:
            if not train_mode and self.eval_loss_fn is not None:
                rank_loss = self.eval_loss_fn.forward_per_list(rank_score, target, length, pnl=pnl)
                return rank_loss
    ...
    def shift_window(self, group_size):
        try:
            self.rank_loss_fn.shift_window(group_size)
        except:
            raise NotImplementedError("shift_window() not implemented for this loss function")
        return
